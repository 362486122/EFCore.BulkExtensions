...
namespace EFCore.BulkExtensions.SQLAdapters.Postgresql
{
    public class PostgresqlAdapter : ISqlOperationsAdapter
    {
        ...
        {
            var connection = OpenAndGetNpgsqlConnection(context, tableInfo.BulkConfig);
            ...
            try
            {
                ...
                var transaction = (NpgsqlTransaction)(context.Database.CurrentTransaction == null ?
            ...
            try
            {
                ...
                using (var importer = connection.BeginBinaryImport(insertCopyString))
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        foreach (var columnName in columnsList)
                        {
                            await importer.WriteAsync(row[columnName]);
                        }
                    }
            ...
            Action<decimal> progress) where T : class
        {
            ...
            try
            {
                ...
                using (var command = GetNpgsqlCommand(context, type, entities, tableInfo, connection, transaction))
                {
        ...
        internal static NpgsqlCommand GetNpgsqlCommand<T>(DbContext context, Type type, IList<T> entities, TableInfo tableInfo, NpgsqlConnection connection, NpgsqlTransaction transaction)
        {
            ...
            foreach (var property in properties)
            {
                if (entityPropertiesDict.ContainsKey(property.Name))
                {
                    ...
                    var parameter = new NpgsqlParameter($"@{columnName}", propertyType); // ,sqliteType // ,null
            ...
            {
                var parameter = new NpgsqlParameter($"@{shadowProperty}", typeof(string));
                command.Parameters.Add(parameter);
            }
        ...
        internal static void LoadSqliteValues<T>(TableInfo tableInfo, T entity, NpgsqlCommand command)
        {
            ...
            foreach (var propertyColumn in propertyColumnsDict)
            {
                ...
                if (!tableInfo.ShadowProperties.Contains(propertyColumn.Key))
                {
                    if (propertyColumn.Key.Contains(".")) // ToDo: change IF clause to check for NavigationProperties, optimise, integrate with same code segment from LoadData method
                    {
                        ...
                        {
                            var parameter = new NpgsqlParameter($"@{propertyColumn.Value}", propertyType);
                            command.Parameters.Add(parameter);
                        }
        ...
        internal static async Task<NpgsqlConnection> OpenAndGetSqliteConnectionAsync(DbContext context, BulkConfig bulkConfig, CancellationToken cancellationToken)
        {
